...
from cartesia import AsyncCartesia, Cartesia, TtsRequestIdSpecifier
...
from cartesia.tts.types.output_format import (
    OutputFormat,
    OutputFormat_Raw,
    OutputFormat_Wav,
    OutputFormat_Mp3,
...
DEFAULT_OUTPUT_FORMAT = OutputFormat_Raw(**DEFAULT_OUTPUT_FORMAT_PARAMS)
...
SAMPLE_VOICE_SPEC = TtsRequestIdSpecifier(mode="id", id=SAMPLE_VOICE_ID)
SAMPLE_TRANSCRIPT = "Hello, world! I'm generating audio on Cartesia."
SAMPLE_LANGUAGE = "en"


def _output_format_to_str(fmt: OutputFormat) -> str:
...
TEST_OUTPUT_FORMATS = [
    ...
    OutputFormat_Wav(container="wav", encoding="pcm_f32le", sample_rate=44100),
    OutputFormat_Wav(container="wav", encoding="pcm_s16le", sample_rate=44100),
    OutputFormat_Wav(container="wav", encoding="pcm_f32le", sample_rate=16000),
    OutputFormat_Wav(container="wav", encoding="pcm_s16le", sample_rate=16000),
    # MP3 format
    OutputFormat_Mp3(container="mp3", sample_rate=44100, bit_rate=128000),
]
...
def _validate_audio_response(data: bytes, output_format: OutputFormat):
...
def test_infill_sync(client: Cartesia, output_format: OutputFormat):
...
@pytest.mark.parametrize("output_format", TEST_OUTPUT_FORMATS)
async def test_infill_async(output_format: OutputFormat):
