...
def _make_connection(ws: MockAsyncWebsocket) -> AsyncTTSResourceConnection_3_0:
    return AsyncTTSResourceConnection_3_0(ws)  # type: ignore[arg-type]
...
# ---------------------------------------------------------------------------
# Tests for the new TTSContextsWSConnection / _AsyncTTSContextsWSConnection
...
class _FakeManagerAttrs:
    """Minimal stub matching the attributes _TTSContextsWSConnection reads off the manager."""
...
def _make_sync_conn() -> Tuple[_TTSContextsWSConnection, _MockSyncInnerConnection]:
    conn = _TTSContextsWSConnection(cast(Any, _FakeManagerAttrs()))
...
def _make_async_conn() -> Tuple[_AsyncTTSContextsWSConnection, _MockAsyncInnerConnection]:
    conn = _AsyncTTSContextsWSConnection(cast(Any, _FakeManagerAttrs()))
...
def test_sync_dispatcher_forwards_user_reconnecting_callback() -> None:
    ...
    conn = _TTSContextsWSConnection(cast(Any, _FakeManagerAttrs(on_reconnecting=user_cb)))
    inner: Any = MagicMock()
